...
# CLI 工具（可選的本地命令列 DOCX/ODT 轉換）
npm run convert             # 執行 run-convert.js 進行轉換
node run-convert.js "path/to/file.docx"
node run-convert.js "path/to/file.odt"
npm run gen-test            # 重新生成 sample test.odt 測試檔
npm run gen-test-docx       # 重新生成 sample test.docx 測試檔
```
...
│   │   ├── docx/
│   │   │   └── docx-parser.js      # Mammoth-based DOCX semantic pipeline & image extraction
│   │   ├── odt/
...
│   │       └── notion-rules.js     # Notion-specific block converters & table cell unwrap
...
├── run-convert.js                  # Standalone CLI tool (supports DOCX and ODT)
├── generate-test-docx.js           # DOCX test fixture generator
├── generate-test-odt.js            # ODT test fixture generator
├── package.json
└── README.md
```
...
3. **DRY CLI Reusability**: `run-convert.js` imports directly from `src/domain/docx/docx-parser.js` and `src/domain/odt/odt-parser.js`.
