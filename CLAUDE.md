...
**Frontend (`public/`) is vanilla JS/CSS/HTML with no build step or framework.** `app.js` talks to `POST /api/convert` (multipart upload), `POST /api/open-folder` (spawns the OS file manager — `cmd.exe /c start` on Windows, `open` on macOS, `xdg-open` on Linux), and `GET /api/history`. Notably, the live preview does **not** use a Markdown library — `renderMarkdown()` in `app.js` is a hand-rolled regex-based Markdown-to-HTML renderer (tables, headings, bold/italic/strike, lists, images rewritten to `/output/<folder>/<path>`). Keep it in sync with whatever Markdown constructs the backend converters can emit.
